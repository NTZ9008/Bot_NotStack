...
-   **Auto Reply** --- ตอบข้อความอัตโนมัติ เช่น "สวัสดีบอท",
    "หิวข้าว"
-   **Daily Weather Report** --- รายงานสภาพอากาศประจำวันแบบ embed พร้อมกราฟพยากรณ์
...
├── welcome/                ← การ์ดต้อนรับแบบรูปภาพ (วาดรูป / API / event สมาชิกเข้า)  
├── weather/                ← รายงานสภาพอากาศประจำวัน (OpenWeatherMap / กราฟ / embed / ตั้งเวลา / API)  
├── db.js  
├── index.js  
├── deploy-command.js  
├── badWords.json  
...
- **พื้นหลัง** — อัปโหลดรูป (PNG / JPG / WEBP / GIF ไม่เกิน 8 MB) เข้าคลังรูปที่ใช้ร่วมกันทุกการ์ด หรือใช้สีพื้น,
...
แทนระบบเดิมที่ส่งอากาศศาลายาเข้าห้อง `GENERAL_CHANNEL_ID` ตอน 07:00 — ครั้งแรกที่บอทเปิดหลังอัปเดต
จะยกค่าเดิม (ห้อง `GENERAL_CHANNEL_ID`, ศาลายา, 07:00 ทุกวัน) มาตั้งให้และเปิดไว้เหมือนเดิม จากนั้นแก้ได้จากหน้าเว็บ

- **embed แบบรายงาน** — หัวข้อ (ลิงก์ไป OpenWeatherMap), วันที่ + สภาพอากาศ, ช่องข้อมูลแถวละ 3 ช่อง และรูปกราฟ
...
### ตั้งค่า
1. `npm run db:migrate` — สร้างตาราง `users`, `refresh_tokens`, `audit_logs`, `activity_events`, `welcome_cards`, `welcome_assets`, `weather_settings`
...
@napi-rs/canvas (วาดการ์ดต้อนรับ — มี binary สำเร็จรูป ไม่ต้องติดตั้งโปรแกรมเพิ่มในเซิร์ฟเวอร์)  
@expo-google-fonts/* (ฟอนต์ไทย Kanit / Prompt / Noto Sans Thai และ Noto Color Emoji)  
